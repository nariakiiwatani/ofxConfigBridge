...
Result Converter::convert(const Document& in, Format to, Document& out, const Options& opt){
    ...
    if (!aFrom) return {false, "no adapter(from)"};
    if (!aTo)   return {false, "no adapter(to)"};

    std::string mid;
    auto r1 = aFrom->dumpText(in, mid, opt);
    if (!r1) return r1;
    return aTo->parseText(mid, out, opt);
}
...
Result Converter::convertText(Format from, Format to, const std::string& in, std::string& out, const Options& opt){
    ...
    if (it == text_.end()) {
        ...
        Document d;
        auto r1 = aFrom->parseText(in, d, opt);
        if (!r1) return r1;
